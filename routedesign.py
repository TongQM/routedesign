...
num_fixed_stops = 15
discount_ratio_array = np.arange(0.1, 1, 0.1)
sizes_array = np.array([25, 50, 75, 100, 150, 200, 300])
...
for size in sizes_array:
    ...
    for discount_ratio in discount_ratio_array:
        num_fixed_stops = int(np.around(size * discount_ratio))
        num_stops.append(num_fixed_stops)
        if size > num_fixed_stops:
    ...
    ax1.legend()
    ax1.set_title(f'size {size} \'s distances')
    fig1.savefig(f'variant_num_stops/demand_size_{size}', format='png')
...
for discount_ratio in discount_ratio_array:
    ...
    ax2.plot(sizes_array[len(sizes_array) - len(tsp_dises):], ondmd_total_dises, label="On demand total distance")
    ax2.plot(sizes_array[len(sizes_array) - len(tsp_dises):], fx_total_dises, label="Fixed route total distance")
    ax2.plot(sizes_array[len(sizes_array) - len(tsp_dises):], wlk_dises, label="Fixed route walk distance")
    ax2.plot(sizes_array[len(sizes_array) - len(tsp_dises):], tsp_dises, label="Fixed route tsp distance")
    plt.xlabel('Num of sizes of demands')
    plt.ylabel('Distances')
    ax2.legend()
    ax2.set_title(f'{num_fixed_stops} with {discount_ratio} as discount stops \'s distances')
    fig2.savefig(f'variant_demand_size/{num_fixed_stops}_stops', format='png')
